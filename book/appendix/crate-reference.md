...
| `regression-diagnostics` | 0.2 | statsmodels-style regression **statistics** (not fitting): `OlsFit::summary()` → R²/adj-R², F-test, AIC/BIC, per-coef t & **VIF**, Durbin-Watson, Jarque-Bera, Breusch-Pagan, White; influence (leverage/Cook's D/DFFITS), QQ data; regularized (`RidgeFit`/`LassoFit`/`ElasticNetFit` — effective df, GCV λ-selection, ridge-VIF); logistic/GLM, survival & mixed models | Newer, single-author; fills the `statsmodels` gap and pairs with `plotters-statistical` (it supplies the numbers, that supplies the charts). **Built on `ndarray` 0.16** — use where `linfa` (0.15) isn't loaded (metrics-deep-dive, regularized-regression). API takes `Array2`/`Array1`; `OlsFit::with_intercept` prepends the intercept column |
...
| `tract-onnx` | 0.22 | **import**: load & run ONNX models (consume models trained elsewhere) | Solid |
| `onnx-export-rs` | 0.1 | **export** canonical Rust models to ONNX — linear / logistic (`Gemm`/`Sigmoid`/`Softmax`), decision-tree / forest / gradient-boosting (`TreeEnsembleRegressor`), SVM, PCA, k-means, k-NN, Naive Bayes; opt-in `smartcore` / `linfa` adapters and Tract round-trip validation | Newer, single-author; the export counterpart to `tract`. Default build pure Rust (`ndarray` 0.16 + `prost`, no `protoc`); params narrow to `f32`. The `validate` feature needs Rust 1.91 (Tract 0.23) |
...
| `chronos-ts` | 0.1 | time-series ARIMA/SARIMA + **`auto_arima`** (order search by AIC/AICc/BIC), `forecast_with_intervals` (80/95% prediction intervals), Prophet-style decomposition, stationarity tests (`adf_test`, `estimate_d`), residual diagnostics (Ljung-Box) | Newer, single-author; complements `augurs` (ARIMA vs exponential smoothing). **`ndarray` 0.15** (matches `linfa`); default features are pure-Rust (`pyo3`/`numpy` are behind an optional `python` feature — don't enable it in evcxr). API takes `Array1<f64>` |
